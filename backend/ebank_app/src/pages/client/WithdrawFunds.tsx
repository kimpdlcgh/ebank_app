...
const WithdrawFunds: React.FC = () => {
  ...
  // Submits the pending withdrawal transaction to Firestore for admin review.
  // ATM/branch withdrawals carry a real dollar amount; the debit-card option in
  // this flow only verifies the PIN (no amount is collected for it), so it does
  // not record a fund movement — it just confirms the card is ready to use.
  const handleConfirmWithdrawal = async () => {
    if (!user?.uid) {
      toast.error('You must be signed in to submit a withdrawal request');
      return;
    }

    const account = activeAccounts.find((acc: any) => (acc.id || acc.accountType) === selectedAccount);
    if (!account) {
      toast.error('Select an account to withdraw from');
      return;
    }

    const requiresAmount = selectedMethod === 'atm' || selectedMethod === 'branch';

    if (!requiresAmount) {
      // Debit card readiness confirmation — no fund movement to record.
      setStep(3);
      return;
    }
    ...
    setSubmitting(true);
    try {
      const methodLabel = selectedMethod === 'atm' ? 'ATM' : 'Branch';
      const atmDetail = selectedMethod === 'atm' && selectedAtm ? ` at ${selectedAtm.name} (${selectedAtm.address})` : '';
      ...
      await addDoc(collection(db, 'transactions'), {
        ...
        description: `${methodLabel} withdrawal${atmDetail}`,
        reference: `WD-${Date.now()}`,
        metadata: {
          method: selectedMethod,
          accountId: account.id,
          atmLocation: selectedMethod === 'atm' && selectedAtm ? { name: selectedAtm.name, address: selectedAtm.address } : null,
        },
