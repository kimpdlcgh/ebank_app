...
const ManageUsers: React.FC = () => {
  ...
  // Optimized Firebase query for users
  const { data: users, loading: isLoadingUsers, error: usersError, refetch } = useUsers(true);
  ...
  useEffect(() => {
    if (usersError) {
      ...
      toast.error('Could not load users from database. Creating new users will still work.');
    }
  }, [usersError]);
  ...
  const resetUserPassword = async () => {
    ...
    try {
      ...
      await createPasswordResetNotification(
        {
          ...
          firstName: selectedUserForReset.firstName,
          lastName: selectedUserForReset.lastName,
      ...
      // Show success message with secure communication instructions
      toast.success(
        `Password reset successful for ${selectedUserForReset.firstName} ${selectedUserForReset.lastName}. ` +
        `Temporary password logged for secure communication.`,
        { duration: 8000 }
      );

      console.log('=== PASSWORD RESET COMPLETED ===');
      console.log(`User: ${selectedUserForReset.firstName} ${selectedUserForReset.lastName}`);
      console.log(`Email: ${selectedUserForReset.email}`);
      console.log(`Username: ${selectedUserForReset.username}`);
      console.log(`Temporary Password: ${newTempPassword}`);
      console.log('Important: User must change password on next login.');
      console.log('=================================');
...
${config.companyInfo.name} Administration Team
      ...
      // Create notification for audit trail
      await createAccountCreationNotification(
        {
          id: firebaseUser.uid,
          firstName: newUserData.firstName,
          lastName: newUserData.lastName,
          username: newUserData.email, // Admin accounts use email as username
          email: newUserData.email,
          role: newUserData.role
        },
        {
                ...
                <div className="space-y-3">
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-sm text-gray-700">
                      <strong>User:</strong> {selectedUserForReset.firstName} {selectedUserForReset.lastName}
                    </p>
                    <p className="text-sm text-gray-700">
                      <strong>Username:</strong> {selectedUserForReset.username}
                    </p>
                    <p className="text-sm text-gray-700">
                      <strong>Email:</strong> {selectedUserForReset.email}
                    </p>
                  </div>
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-xs">
                      ...
                      <li>You must communicate the password to the user securely</li>
                    </ul>
                  </div>
                </div>
              </div>
