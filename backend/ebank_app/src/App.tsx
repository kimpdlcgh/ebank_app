...
const DEMO_BROKERAGE_PREVIEW = false;
const AppRouter = import.meta.env.PROD ? BrowserRouter : Router;
...
const WithdrawFundsPage = React.lazy(() => import('./pages/client/WithdrawFunds'));
...
const ManageInstruments = React.lazy(() => import('./pages/admin/ManageInstruments'));
const ManageBrokerageOrders = React.lazy(() => import('./pages/admin/ManageBrokerageOrders'));
const ManageClients = React.lazy(() => import('./pages/admin/ManageClients'));
...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, requiredRole }) => {
  ...
  if (!user) {
    if (DEMO_BROKERAGE_PREVIEW && requiredRole === UserRole.CLIENT) {
      return <>{children}</>;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Suspense fallback={<LoadingSpinner />}>
        ...
        />
        <Route
          path="/admin/brokerage-orders"
          element={
            <ProtectedRoute requiredRole={UserRole.ADMIN}>
              <ManageBrokerageOrders />
            </ProtectedRoute>
          }
        />
        <Route
        ...
        {/* Default redirect to brokerage dashboard for demo preview */}
        <Route path="/" element={<Navigate to={DEMO_BROKERAGE_PREVIEW ? "/dashboard" : "/client-login"} replace />} />
