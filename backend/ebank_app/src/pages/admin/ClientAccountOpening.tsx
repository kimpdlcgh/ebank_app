...
import { useAccountCategories } from '../../hooks/useFirestore';
...
// Fallback account types used while the admin-managed categories are loading
// or if the `account_categories` collection is empty, so this step never renders blank.
...
const ClientAccountOpening: React.FC = () => {
  ...
  const { data: accountCategories, loading: accountCategoriesLoading } = useAccountCategories();
  ...
  // Account types offered on Step 5, sourced from the admin-managed `account_categories`
  // collection. Falls back to the original hardcoded list while loading or if empty.
  const enabledAccountCategories = (accountCategories || []).filter((category: any) => category.enabled !== false);
  const accountTypeOptions: typeof fallbackCategories = (!accountCategoriesLoading && enabledAccountCategories.length > 0)
    ? enabledAccountCategories.map((category: any) => ({
        type: category.code as string,
        title: category.name as string,
        description: category.description as string,
        features: (category.features || []) as string[],
        minDeposit: (category.minimumDeposit ?? 0) as number
      }))
    : fallbackCategories;
